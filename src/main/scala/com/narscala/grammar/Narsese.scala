...
class Narsese(val input: ParserInput) extends Parser with StringBuilding {
    ...
    */
    ...
    def num:Rule1[Double] = rule { capture(digits ~ optional(dec)) ~> ((s:String) => s.toDouble) }
    ...
    def compoundterm:Rule1[CompoundTerm] = rule {
        ...
        "(" ~ termconnector ~ "," ~ oneOrMore(term).separatedBy(",") ~ ")" ~> ConnectedSet // See Op
    }
