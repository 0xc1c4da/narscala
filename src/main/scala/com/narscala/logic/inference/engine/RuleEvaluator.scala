...
private[engine] abstract class RuleManipulator(workingMemory: WorkingMemory) {
  ...
  def produce_once(item: Any) = workingMemory produce_once item
...
private[engine] class RuleEvaluator(rule: Rule, workingMemory: WorkingMemory)
  ...
  private[engine] def registerExecution(executedSet: RuleExecutionSet) = {
...
private[engine] class RuleExecutor(evaluator: RuleEvaluator, rule: Rule, workingMemory: WorkingMemory, executionSet: RuleExecutionSet)
                  extends RuleManipulator(workingMemory) with Logger {
...
private class ValuesCombinator {
  ...
  private[engine] def hasNext() = !values.isEmpty && !finished

  private[engine] def +=[A] (t: Traversable[A]): Option[A] = {
    values = t.toList :: values
    finished = t.isEmpty
    if (finished) None else Some(t.head)
  }

  private[engine] def next[A](clazz: Class[A]): Option[A] = {
