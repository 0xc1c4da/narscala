...
private[engine] case class RuleDefinition[A](condition: () => Boolean, execution: () => A)

object Rule {

  type EvaluationContext = ThreadLocal[RuleManipulator]
  private[engine] val evaluationContext = new EvaluationContext
  ...
  def when(condition: => Boolean) = new {
    def perform[A](execution: => A) = RuleDefinition(condition _, execution _)
  }
  def perform[A](execution: => A) = RuleDefinition(() => true, execution _)
  ...
  def produce_once(item: Any) = currentContext produce_once item
