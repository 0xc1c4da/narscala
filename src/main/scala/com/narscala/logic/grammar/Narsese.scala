...
/** Narsese Parser that attempts to reflect;
  ...
  * TODO: Instance & Property copula transform to ExtensionalSet & IntensionalSet
  */
class Narsese(val input: ParserInput) extends Parser with StringBuilding {
    ...
    def copula:Rule1[Copula] = rule {
        ...
        // ("{--" | "◦→" )  ~> Instance             |                               // instance, NAL-2, no need to implement, use ExtensionalSet instead
        // ("--]" | "→◦" )  ~> Property             |                               // property, NAL-2, no need to implement, use IntensionalSet instead
